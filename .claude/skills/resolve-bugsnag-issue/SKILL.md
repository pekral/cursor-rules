...
- Resolve this issue (the generated code must be according to @.cursor/skills/class-refactoring/SKILL.md), then review the code according to @.cursor/skills/code-review/SKILL.md and @.cursor/skills/security-review/SKILL.md for current changes. If DB-related changes are present, run @.cursor/skills/mysql-problem-solver/SKILL.md (real SQL dump + EXPLAIN for every query + schema check from migrations/dev/test DB) and include all SQL findings with severity **Critical** and **Moderate** in the CR output and fix loop. If you find any critical issues in the new changes, resolve them and perform further iterations of the defined code review (repeat until the bug is fixed).
