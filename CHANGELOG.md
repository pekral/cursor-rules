...
- ✨ **Added**: new Agent skill `assignment-compliance-check` — plain-language verification that the PR implementation satisfies the linked issue's business requirements. Writes a non-technical markdown report to `${HOME}/.cursor-rules-reports/assignment-compliance/` and is invoked from every CR run (`code-review`, `code-review-github`, `code-review-jira`) so its Critical findings appear in the published CR comment (#457)
