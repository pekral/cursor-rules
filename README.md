...
| `daidalos` | Engineering-workflow orchestrator. The entry point for a free-form request: resolves a concrete source, then **dispatches** `metis` (analysis, if needed), `talos` (implementation), `apollon` (fast scoped validation after each landing step), `argos` (code quality / architecture / optimisation CR) and `athena` (security CR, parallel to `argos`) through the Task tool. Read-only orchestrator. | `metis`, `talos`, `apollon`, `argos`, `athena` (dispatched) |
