...
| `apollon` | Test engineer. Designs test scenarios (edge cases, regression) from the assignment, writes PHPUnit/Pest tests, generates browser test scenarios, verifies the acceptance criteria, and hunts broken flows — understanding both the code and the product assignment. Hands back a Tests-done handoff. Write-capable for test code only — never touches application code or merges. | `create-test`, `create-missing-tests-in-pr`, `e2e-testing`, `test-like-human` |
