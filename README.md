...
| `daidalos` | Engineering-workflow orchestrator. The entry point for a free-form request: resolves a concrete source, then **dispatches** `metis` (analysis, if needed), `talos` (implementation), `apollon` (fast scoped validation after each landing step), and `argos` (the review-and-fix loop to convergence) through the Task tool. Read-only orchestrator. | `metis`, `talos`, `apollon`, `argos` (dispatched) |
| `apollon` | Test engineer and fast scoped validation gate. On-demand: designs test scenarios, writes PHPUnit/Pest tests, generates browser scenarios, verifies acceptance criteria, hunts broken flows. Push-level gate (auto): `daidalos` dispatches it after talos PR-open and after argos convergence — it runs only the tests covering the diff, verifies the relevant acceptance criteria against the diff, and uses full `composer build` only for broad changes. Hands back a `Tests done` or `Tests done (scoped)` handoff. Write-capable for test code only — never touches application code or merges. | `create-test`, `create-missing-tests-in-pr`, `e2e-testing`, `test-like-human` |
