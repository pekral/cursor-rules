...
| `daidalos` | Engineering-workflow orchestrator. The entry point for a free-form request: resolves a concrete source, then **dispatches** `metis` (analysis, if needed; or decomposition of a broad subject into multiple structured issues — after which it reports the issues and stops, no PR), `talos` (implementation), `apollon` (fast scoped validation after each landing step), `argos` (code quality / architecture / optimisation CR) and `athena` (security CR, parallel to `argos`) through the Task tool. Plans dependency-aware resolve order when issues are interlinked. Read-only orchestrator. | `metis`, `talos`, `apollon`, `argos`, `athena` (dispatched) |
...
| `athena` | Strategic security CR sentinel. Dispatched by `daidalos` **in parallel with `argos`** on the same PR — runs all security skills (security-review, laravel-security, security-bounty-hunter, security-threat-analysis), applies all security rules (backend, frontend, mobile), labels each finding (Critical / Moderate / Minor), posts the consolidated security review to the PR, and hands back a Security-CR-done handoff. Active only after installer registration — fallback: security runs inline in `code-review-github → security-review`. Read-only. | `security-review`, `laravel-security`, `security-bounty-hunter`, `security-threat-analysis` |
| `keryx` | Release announcer / publicista. Give it a merged change or release — from context or a tracker link — and it composes announcement content: a Twitter/X tweet (≤280 chars) + thread, release notes, and a marketing summary with pekral.cz promotion. Runs post-delivery, outside the CR loop. Publishes only when explicitly asked and only through the canonical wrapper. Read-only. | `article-writing`, `resolve-issue/references/source-detection` |
