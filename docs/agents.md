...
- **`tools`** — restrict to what the agent needs. A read-only reviewer needs `Read, Glob, Grep, Bash` only; `argos`, `athena`, and `metis` additionally carry `WebSearch, WebFetch` to verify third-party API documentation and research authoritative sources — neither tool writes to the working tree, so the read-only stance still holds.
