...
- **Hand over a logically partitioned history.** The coverage commit, the pre-existing bug fix, the refactor itself, and any follow-up tests are separate commits by the rules above — before publishing, verify `git log` actually reflects that split and reshape it when it does not (split a bundled commit, fixup a scattered one, amend a subject that no longer describes its contents, add the commit a change never got), per `@rules/git/general.mdc` *Git Rules*.
...
- **A comment is a refactoring signal, not a deliverable.** Whenever the touched code needs a comment to be followed, treat that as evidence the code is not readable enough and refactor it away — rename the symbol the comment was clarifying, extract one intention-revealing method per commented step, split a mixed-responsibility method, or replace the magic value with a named constant / enum case — then delete the narration the restructuring made redundant. Keep a comment **only** when it carries the *why* / a domain definition / a navigation pointer the code cannot express, or when the logic is genuinely complex and that complexity is irreducible, per `@rules/php/core-standards.mdc` *Documentation*. Never propose merely rewording or shortening a comment while leaving the unreadable block intact, and never add a comment as the fix for code you could have made clearer. **This applies to the comments already in the class, not only to the ones the refactor would add:** a class you restructure is delivered with its pre-existing *what*-narration removed — the restatements of the signature, the redundant type docblocks, the section banners, the commented-out code, the descriptions the restructuring just made false. Leaving them is not "preserving behavior", it is shipping a rewritten class still explained by prose written for the old one. In `MODE=cr`, surface each such surviving comment as part of the refactoring proposal for the block it sits in, never as a separate repo-wide comment sweep.
...
- **Service → Action conversion (Laravel + `pekral/arch-app-services`).** Per `@rules/laravel/architecture.mdc` *Model Services*, a Service-role class that does not extend `BaseModelService` — and in particular one exposing exactly **one** public method with the rest private — is an Action wearing a Service name. Propose the conversion: move it to `app/Actions/{Domain}/`, rename the single public method to `__invoke()`, keep the helpers private, and update every call site to `$action($payload)`. In `MODE=cr`, emit it as a written proposal listing the call sites rather than applying the change.
- **Pass-through Actions (Action pattern).** Per `@rules/laravel/architecture.mdc` *Pass-through Action rule*, an Action whose entire `__invoke()` body is a single delegating call to **one collaborator** — a Model Service method **or another Action** — with no orchestration of its own (no validation delegation, no DTO / data transformation, no coordination of multiple collaborators, no extra business step, no return-value reshaping) — is a redundant indirection layer and must be collapsed during the refactor. Detect every such pass-through Action touched by the refactor and resolve it by the collaborator it forwards to: (1) the wrapped Service method is used **only once** in the codebase → move its logic into the Action and delete the method (the **Single-use Service method rule**), so the Action does real work; (2) the method is **reused** elsewhere → remove the Action entirely and rewrite the entry point to call the Service method directly (`$action($payload)` → `$service->method($payload)`), updating every call site; (3) the body forwards to **another Action** → delete the **outer** Action and invoke the inner one from the entry point (`$outerAction($payload)` → `$innerAction($payload)`), or merge the two into one Action when the outer name states the use case better and the inner has no other caller — never keep both. A body that adapts around the single call (input mapping, return reshaping, validation delegation, transaction / lock / authorization wrapping) is orchestration, not a pass-through. In `MODE=cr`, emit each pass-through Action as a written refactoring proposal (target resolution + every call site that must change) rather than applying the change.
