...
- Run conditionally (same triggers as `@skills/code-review-jira/SKILL.md`): refactoring diff → `@skills/refactor-entry-point-to-action/SKILL.md` with `MODE=cr`; database operations → `@skills/mysql-problem-solver/SKILL.md` (surface under `## Database Analysis`); third-party API changes → the Third-Party API & Service Analysis step from `@skills/code-review/SKILL.md`, including its mandatory documentation verification (locate the docs or request the link — never assess from memory).
