...
- **Report severity filter from iteration 3.** When the caller passes `iteration = <n>` and `n > 2`, the posted PR comment carries **Critical and Moderate findings only** — drop every Minor finding (in `Findings`, `Architecture`, `Database Analysis`, and every specialized-review sub-section), the whole `Refactoring (DRY / Tech Debt Reduction)` section, and the whole `Refactoring Proposals` section, and carry `report filter: critical+moderate only (iteration {n})` on the Counts and summary lines. No walk or sub-review is skipped and the Critical + Moderate merge gate is unchanged. Canonical: `@rules/code-review/general.mdc` *CR report severity filter from iteration 3 (Critical & Moderate only)*.
