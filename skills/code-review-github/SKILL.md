...
- **`## Architecture` section is the second exception (issue #530).** On Laravel projects (`laravel/framework` is in `composer.json` `require`), the mandatory architecture walk-through defined in `@skills/code-review/SKILL.md` Core Analysis always reports — render the `## Architecture` heading on every CR run, with the listed findings when any exist or with the literal status line `Status: walked, 0 findings` when the walk is clean. On non-Laravel projects, omit the `## Architecture` section entirely.
