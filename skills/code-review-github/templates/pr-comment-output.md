...
> **Report severity filter from iteration 3 (`iteration > 2`).** When the CR wrapper is invoked with `iteration = <n>` from the Review loop of `@skills/process-code-review/SKILL.md` and `n > 2`, render **Critical and Moderate findings only**: drop every Minor finding (in `## Findings`, `## Architecture`, `## Database Analysis`, and every specialized-review sub-section), drop the whole `## Refactoring (DRY / tech debt)` section, and drop the whole `## Refactoring proposals` section. The `Counts` and `Summary` lines then carry only the rendered severities plus the marker `report filter: critical+moderate only (iteration {n})`; never emit a placeholder for a filtered-away item and never report it as a zero count. A run with no `iteration` input is `iteration = 1` and renders the full report. Canonical: `@rules/code-review/general.mdc` *CR report severity filter from iteration 3 (Critical & Moderate only)*.
