...
> **Subagent dispatch (preferred).** Each independent sub-review below should run **inside its own subagent** via the `Agent` tool (`subagent_type: "general-purpose"`) — dispatch the always-run set in a **single message with parallel `Agent` tool calls** so they execute concurrently and their findings come back without polluting this wrapper's context. Each subagent prompt must (a) state the skill to run (`@skills/<name>/SKILL.md` with any `MODE=cr` flag), (b) pass the PR URL / number and the branch already checked out, (c) declare the publishing contract for this CR run (quiet vs publish; see step 4), and (d) require the subagent to return its findings as the canonical markdown block (`## Assignment Compliance` block, Critical / Moderate / Minor lists with reproducer fields, refactoring proposals). The CR wrapper then assembles the subagent outputs into the final GitHub PR comment + JIRA / linked-issue summary. Fall back to invoking the skills in-line only when subagent dispatch is unavailable.
>
> The mysql-problem-solver / race-condition-review / refactor-entry-point-to-action conditionals follow the same rule: when their trigger fires, dispatch them as additional parallel subagents alongside the always-run set so the entire review fan-out happens in one round.

- For each PR (each sub-review in its own subagent, dispatched in parallel):
...
- **Consolidation contract (issue #498):** pass the GitHub-Markdown version of the `## Assignment Compliance` markdown block returned by `@skills/assignment-compliance-check/SKILL.md` as an embedded block so the linked-GitHub-issue audience also sees **one consolidated comment per CR run** instead of two separate posts. Follow-up runs add a new comment rather than editing the previous one — the chronological sequence is the audit trail.
...
- The posted PR comment must always include a `## Coverage` section before the summary line. The section reports the coverage command run (per the Coverage gate in `@skills/code-review/SKILL.md` — the project's available coverage tooling, scoped to the changed files), the exact command run, and the coverage result for the changed files (or "coverage tooling unavailable" with reason). Never post a CR comment without this section.
