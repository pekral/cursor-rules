...
- For each PR (sub-reviews invoked inline, one at a time):
  ...
  - run @skills/class-refactoring/SKILL.md **with `MODE=cr`** — read-only refactoring lens scoped to the PR diff, **run on every CR**. The lens walks the skill's **complete guideline set** (its *Refactoring Guidelines*, *Laravel Context*, and the behavior-preservation clauses of *Execution*) against the lines the PR touched — DRY duplication and oversized methods are the common case, not the boundary of the walk. **Every item it returns is rendered in the published GitHub PR comment**, routed and de-duplicated per `@rules/code-review/general.mdc` *Refactoring & Tech Debt (DRY) Analysis — diff-scoped detail*: to its severity bucket when the underlying rule declares one, otherwise to **Refactoring (DRY / Tech Debt Reduction)** / **Refactoring Proposals**. The JIRA non-technical comment never carries these technical sections. Canonical definition of the lens invocation lives in `@skills/code-review/SKILL.md` Specialized Reviews → Always run. `MODE=cr` guarantees no code changes, commits, fixers, or review chaining.
...
2. For each changed block, apply `@skills/class-refactoring/SKILL.md` (run with `MODE=cr` — read-only), walking that skill's **complete guideline set**. The list below is the **high-frequency subset, not the closed set** — a guideline defined in the skill but absent here (speculative interfaces, pass-through Actions, `>4` parameters → DTO, Simplicity First, business-logic-layer placement, Repository scope, query performance non-regression, intention-revealing extraction, naming, nesting depth, …) is still walked and still raises an item:
...
5. **Every item the lens returns reaches the GitHub PR comment** — an item whose underlying rule declares a severity is routed to that Critical / Moderate / Minor bucket (with the four reproducer fields) instead of the Refactoring section, and a violation already raised by the Core Analysis / Strict rule compliance walk is kept there rather than duplicated. The full routing and no-drop contract is owned by `@rules/code-review/general.mdc` *Refactoring & Tech Debt (DRY) Analysis — diff-scoped detail*.
