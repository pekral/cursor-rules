...
- Run conditionally:
  ...
  - Third-party API or service changes → ensure the **Third-Party API & Service Analysis** step from `@skills/code-review/SKILL.md` is executed for the diff, including its mandatory documentation verification (locate the docs or request the link — never assess from memory)
...
- **Clarifying questions block (conditional).** While running the sub-reviews, collect every **genuine open question** the reviewer needs answered before the work can be accepted — an ambiguity in the assignment that the issue description, comments, and code could not resolve (a missing acceptance criterion, an undefined edge case, a value the assignment never specified, a contradiction between the ticket and a comment). Then assemble the block exactly as `@rules/code-review/general.mdc` *Clarifying Questions — Answered-Question Suppression & Severity Gate (issue #758)* defines it: run the mandatory **answer walk** over **all** tracker comments (`skills/code-review-jira/scripts/parse-comments.sh <KEY|URL>`, plus `skills/code-review-github/scripts/parse-comments.sh` on the PR and every `closingIssues[]` entry) before asking anything, **drop every question the thread already answered and the diff implements**, re-route an answered-but-unimplemented requirement to a **Critical** finding on the GitHub PR comment instead, and render **only Critical and Moderate** questions — Minor ones are dropped silently. When at least one question survives, render it as a `h2. Clarifying questions` block in JIRA Wiki Markup (one `*` bullet per question, each a single plain-language sentence, **no severity label and no count**) and pass it as an embedded block to `pr-summary` so it renders after `How to test`. When none survives, pass nothing — never emit an empty "no questions" block. Do not invent questions to fill the section; ask only what genuinely blocks acceptance.
...
- **`## Commit Split Proposal` section (issue #763).** The **Commit Split & Atomic Deployability Gate** (`@skills/code-review/SKILL.md`, canonical detail in `@rules/code-review/general.mdc` *Commit Split & Atomic Deployability Proposal — canonical walk-through*) runs on every CR run over `git log <base>..<head>` and each commit's own diff. When the history is **not** already a logical, independently cherry-pickable partition of the change set, render the `## Commit Split Proposal` section immediately after `## Findings` — verdict, one row per proposed commit (subject, contents, source hunks, assignment item, cherry-pick verdict, reversibility), rollback notes, the reconciliation proving the union equals the current diff, and the blockers — and add the `commit split: N commit(s) proposed` slot to the summary line. The finding is **Critical**, always: never downgraded, never demoted into a refactoring section, never dropped by the late-iteration filter, and exempt from the Faulty Example / Expected Behavior / Test Hint fields. The proposal is a repartition only — the union of the proposed commits equals the current diff byte for byte and `git diff <old-head> HEAD` is empty after the rewrite; this skill is read-only and **never** rewrites history itself. When the history is already atomic, omit the section and the slot entirely.
- **Late-iteration report scope (CR iteration > 2).** When the caller passes `iteration = <n>` and `n > 2`, the posted GitHub PR comment carries **Critical and Moderate findings only** — drop every Minor finding (in `Findings`, `Architecture`, `Database Analysis`, and every specialized-review sub-section), the whole `Refactoring (DRY / tech debt)` section, and the whole `Refactoring proposals` section, and render the header line `**Report scope:** critical+moderate only (iteration {n}) — Minor findings and refactoring sections not rendered` while the Counts and summary lines keep their real, unchanged numbers. No walk or sub-review is skipped and the Critical + Moderate merge gate is unchanged. Canonical: `@rules/code-review/general.mdc` *Late-Iteration Report Scope — Critical & Moderate Only (CR iteration > 2)*.
...
- When the diff touches database operations (per the trigger list in `@skills/code-review/SKILL.md` Specialized Reviews), the posted GitHub PR comment must include a dedicated `## Database Analysis` section **before** `## Coverage`. The section reports only the `mysql-problem-solver` findings (with severity mirroring Critical / Moderate / Minor), each carrying a **Suggested Fix that renders the concrete optimization artifact** — the rewritten query in full, the exact index DDL, or the rewritten batching code in a fenced code block — or, when the slowdown is the justified branch, the three-part documentation block that **is** the artifact there. Naming the fix category in prose does not satisfy the requirement on its own, per `@rules/code-review/general.mdc` *Database Analysis section* and `@rules/sql/optimalize.mdc`. Each **DB-performance** defect appears here exactly once and is never duplicated into the Critical / Moderate / Minor buckets; a **security** finding on the same `file:line` is a different defect and always keeps its own entry in the severity buckets. Do not include the queries / migrations inspected list or any EXPLAIN / static-analysis summary — those stay inside the internal investigation. When no DB operations are present, omit the section entirely. The JIRA non-technical comment (produced by `pr-summary`) never includes this section.
...
- The JIRA comment carries **only `How to test`**, plus the two conditional embedded blocks when they apply — *Clarifying questions* (open questions the reviewer needs answered, filtered per `@rules/code-review/general.mdc` *Clarifying Questions — Answered-Question Suppression & Severity Gate*: questions the tracker thread already answered and the diff implements are dropped, and only Critical / Moderate ones are rendered, unlabelled) and *Assignment Compliance* (assignment discrepancies + Critical items). It carries no `Authors` line, no `Summary of changes` section, no severity counts, no file paths, no line numbers, no code snippets — `pr-summary` enforces this reduced shape by design. Plain language understandable by non-developers.
