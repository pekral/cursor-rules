...
- **Consolidation contract (issue #498):** invoke `pr-summary` exactly once for the JIRA ticket, passing the conditional blocks together (clarifying questions first, then assignment compliance) so the reader gets **one** comment. When `@skills/assignment-compliance-check/SKILL.md` returned a markdown block (i.e. at least one Critical gap was detected), first convert that block to JIRA Wiki Markup per `@rules/jira/general.mdc` (`## ` → `h2. `, `**bold**` → `*bold*`, `` `code` `` → `{{code}}`, `- ` → `* `, `[label](url)` → `[label|url]`) and pass it as an embedded block — `pr-summary` appends it verbatim after `How to test`. When the compliance check returned the `no critical gaps — assignment compliance block omitted` status, **do not** pass that block; clean compliance is reported by its absence (no "Verdict: clean" line, no "What is satisfied" list). `pr-summary` then **upserts** the JIRA comment via `skills/code-review-jira/scripts/upsert-comment.sh` (JIRA MCP server fallback when `acli` lacks comment-edit support or exits with code 4). The upsert keeps **exactly one JIRA comment per (ticket, acli actor)** — keyed by the invisible anchor `{anchor:cr-comment-actor-<slug>}` (auto-prepended by the upsert helper) — so follow-up runs edit it in place instead of stacking new comments. Never post a separate JIRA comment for clarifying questions or assignment compliance on top of it.
...
- **Verify the published JIRA body contains no leaked Markdown.** Before / when publishing, confirm the body uses only Wiki Markup — no `**` / `__`, no `#`/`##` ATX headings, no `` ` ``/```` ``` ````, no `- ` bullets, no `[label](url)` links. Any such artifact must be converted per `@rules/jira/general.mdc` so the JIRA UI never shows raw markup characters.
