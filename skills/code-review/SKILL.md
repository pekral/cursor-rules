...
- **Method parameter count (>4 → DTO required, strict):** flag every method, function, closure, constructor, `__invoke()`, or other callable whose signature declares **more than 4 parameters** on any line added or modified by the diff. Promoted constructor properties count as parameters; a variadic (`...$args`) counts as one parameter. The mandatory fix is to introduce a dedicated typed DTO (preferably `final readonly` with `public function __construct(...)` promotion) and pass it as a single argument, per `@rules/php/core-standards.mdc` Structure section. Severity: **Critical** (structural / required-pattern violation; the Strict rule compliance default applies and may not be silently downgraded). The only exempt cases are entry-point signatures fixed by an external framework / vendor contract that the project cannot change (controller actions whose argument list is resolved by the service container, magic methods such as `__construct` of an `Illuminate\Console\Command` subclass when bound by Laravel, event listeners whose signature is dictated by the dispatched event, methods overriding a parent / interface whose signature is owned by `vendor/`); the exemption must be cited explicitly in the finding with the parent class / interface and a one-line reason. Existing methods that the diff does **not** touch are out of scope; a pre-existing method becomes in scope the moment the diff adds, removes, renames, or re-types any parameter of that method.
