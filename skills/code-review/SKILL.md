...
- **Coverage gate (mandatory, diff-scoped only):** every line, branch, and condition added or modified in the current PR diff must be covered by tests. The CR coverage step **must execute the diff-scoped script alone** — never the project-wide coverage command (`composer test:coverage`, `composer coverage`, full Phing `coverage` target, `pest --coverage --min=100` on the whole suite, etc.).
  - Discover the diff-scoped command in this order: (1) Phing target named `test:coverage:diff` / `coverage:diff` in `build.xml` / `phing.xml`; (2) Composer script `test:coverage:diff` in `composer.json` (this repository ships one — `composer test:coverage:diff` → `bin/test-coverage-diff`, which auto-picks PCOV when loaded and falls back to Xdebug, and auto-detects Laravel projects to cover `app/` + `src/` vs `src/` for non-framework projects); (3) any project-specific script whose name contains `diff` and `coverage`. Do not assume a default and do not fall back to a full-suite coverage command.
...
- This skill is read-only and does not publish anywhere itself. The wrapper skills that consume its output (`@skills/code-review-github/SKILL.md`, `@skills/code-review-jira/SKILL.md`) **must** delegate the non-technical summary on **every linked issue** in the originating tracker (GitHub issue, JIRA ticket, or both) to `@skills/pr-summary/SKILL.md` — the CR wrappers never author their own non-technical template. `pr-summary` produces a uniform *"Summary of changes + How to test"* comment understandable by non-technical project managers, rendered as GitHub Markdown for GitHub issues and JIRA Wiki Markup for JIRA tickets per `@rules/jira/general.mdc`. Technical findings still go directly on the PR comment.
...
- **Database Analysis section (mandatory when the diff touches DB operations):** when the conditional `@skills/mysql-problem-solver/SKILL.md` trigger fires (see **Specialized Reviews** for the trigger pattern list), append a dedicated `## Database Analysis` section to the published review **before** the `## Coverage` section. The section reports: the queries / Eloquent statements / migrations / schema changes inspected (file:line list), the EXPLAIN or static-analysis outcome (when DB access is available, otherwise note the limitation), each finding with severity (mirrors the **Critical / Moderate / Minor** scale) plus the proposed query rewrite / index reuse / batching fix per `@rules/sql/optimalize.mdc`, and the call decision (DB changes detected vs. trigger skipped). When no DB operations are present in the diff, omit the section entirely; never replace it with a generic "no DB changes" placeholder absorbed into Coverage or the summary line.
