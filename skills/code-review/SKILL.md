...
- **Report severity filter from iteration 3.** When the caller passes `iteration = <n>` (the Review loop iteration from `@skills/process-code-review/SKILL.md`) and `n > 2`, the report carries **Critical and Moderate findings only** — every Minor finding, `Refactoring (DRY / tech debt)`, and `Refactoring proposals` are dropped at Output assembly. No walk is skipped and the convergence gate is unchanged. Canonical: `@rules/code-review/general.mdc` *CR report severity filter from iteration 3 (Critical & Moderate only)*.
