...
- Always run:
    ...
    - @skills/class-refactoring/SKILL.md **with `MODE=cr`** — read-only refactoring lens scoped to the PR diff, **run on every CR**. The lens walks the skill's **complete guideline set** (every bullet of its *Refactoring Guidelines* and *Laravel Context* sections, plus the behavior-preservation clauses of its *Execution* section) against the lines the PR actually touched — DRY duplication, single-responsibility breaches, and oversized methods are the common case, **not** the boundary of the walk: speculative interfaces, pass-through Actions, `>4` parameters → DTO, Simplicity First, business-logic-layer placement, Repository scope, query performance non-regression, intention-revealing extraction, naming, and nesting depth are walked too. **Every item the lens returns is rendered in the published report** — routed to its severity bucket when the underlying rule declares a severity, otherwise to **Refactoring (DRY / tech debt)** / **Refactoring proposals**, and de-duplicated against the Core Analysis / Strict rule compliance walks so each violation is raised exactly once. The routing and no-drop contract lives in `@rules/code-review/general.mdc` *Refactoring & Tech Debt (DRY) Analysis — diff-scoped detail*. `MODE=cr` guarantees the lens never modifies code, writes tests, commits, runs fixers, or chains another review — it returns proposals only. Do not propose changes that fall outside the diff.
