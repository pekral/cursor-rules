...
- **Custom Rule classes reuse (**Moderate**):** When reviewing FormRequests, scan the project's `app/Rules/` directory for existing custom Rule classes. If a FormRequest contains inline validation logic that duplicates or reimplements logic already covered by an existing custom Rule class, flag as **Moderate** and recommend using the existing Rule class instead. This applies to both new and modified FormRequests.
