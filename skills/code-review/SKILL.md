...
Walk both directions and record the verdict per `@rules/code-review/general.mdc` *Assignment Conformance Gate — canonical detail*: **1. Requirements → changes (completeness)** — consume the result of the always-run `@skills/assignment-compliance-check/SKILL.md` and `@skills/analyze-problem/SKILL.md`, never re-derive it; **2. Changes → requirements (traceability, no scope creep)** — trace every changed block back to a requirement and raise an out-of-scope finding for each block that traces to none; **3. Verdict** — `assignment conformance: conformant` / `N gap(s)`, computed at Output assembly so it never contradicts the Counts line.
...
- **Full Core Analysis walk-through (canonical detail in `@rules/code-review/general.mdc` *Core Analysis Walk-through*).** Apply every bullet there to the diff and raise one finding per violation at the severity it declares: Reuse of existing logic, Action scope, Speculative interfaces, **Simplicity First**, method-parameter-count (>4 → DTO), public-method raw-array-vs-DTO, new static-analysis / linter suppression, **Strict rule compliance (mandatory walk-through)**, **Architecture conformance (Laravel)** (issue #530), **Test organization (issue #528)**, **Queue assertion specificity (issue #756)**, per-row DB operations in loops, **batch-first processing & performance at scale**, variable ordering / lazy evaluation, object caching, new storage reuse analysis, SQL index reuse / performance non-regression, refactoring quality + test-coverage contract, data-validation encapsulation, pass-through Action, repository scope, inline Eloquent / read-write layer separation, Action-returns-HTTP-response, inline data mapping → Data Builder, inline validation guards / `throw_if` / `throw_unless` / enum-mode `match()` → Data Validator, only-Laravel-and-arch-layers class inventory, **Request → DTO transformation belongs in the FormRequest, not the controller**, Data Modification (DRY), **Entry-point error handling for known failures (Laravel)**, **New PHP file — architecture & design conformance** (issue #763), **Commit split & atomic deployability** (issue #763), and **Self-Documenting Code — Comment & Doc Hygiene** (scope covers the comments the diff adds or modifies **plus the pre-existing ones inside the region it passes through** — the changed hunks and their enclosing method / class member, never an untouched method or file; a **stale comment on a line the diff itself touches** is **Moderate** and blocks, its other findings — pre-existing noise included — stay Minor).

### New PHP File Architecture Gate (mandatory)

Every **new** `*.php` file the diff adds under the project's production source roots is reviewed as a design decision, not only as content: verify it lands in a layer the project's architecture rules actually name, that namespace / path / class name agree, that the class carries the shape its layer requires, that its name states the domain role, that it has one reason to change, that its dependency direction holds, that it declares `strict_types` with typed boundaries, that it duplicates no existing class in the same layer, and that it is wired up. Every finding of this walk is **Moderate** — never downgraded to Minor, never escalated from inside the walk. Framework-generated files in their own directories, test files (owned by **Test organization**), documented layer choices, and files the diff only modifies are not findings. On a Laravel project the **Architecture conformance** walk owns the Critical dimensions and wins on the same file; on a non-Laravel PHP project this gate is the only architecture walk that runs. Canonical checks, non-findings, gating, and Suggested-Fix templates: `@rules/code-review/general.mdc` *New PHP File — Architecture & Design Conformance (issue #763)*.

### Commit Split & Atomic Deployability Gate (mandatory)

Every CR run reviews the **shape of the commit history**, not only the squashed diff: read `git log --reverse --format='%H%n%s%n%b' <base>..<head>` and each commit's own diff on the checked-out branch, and judge whether the history is already a logical, independently cherry-pickable partition of the change set per `@rules/git/general.mdc` *Git Rules*. When it is not, render the repartition as the `## Commit Split Proposal` section, raise it as a **Critical** finding, and add the `commit split: N commit(s) proposed` slot to the `Summary` line. The proposal is a **repartition only** — the union of the proposed commits equals the current diff byte for byte (`git diff <old-head> HEAD` empty after the rewrite), no production line is added / removed / rewritten, no work deferred. It is also a **green** partition: a commit that is red at its own point of the history — it does not build, or it carries a failing / skipped / simulated-failing test a later commit turns green — is itself a trigger, and no proposed commit may be red (hunks that cannot be separated without a red intermediate stay in one commit), so the Suggested Fix's rewrite is verified per commit with `git rebase --exec`, never at the tip alone. It is also a **live** partition: a commit that ships dead code — a class, method, branch, route, config key, or asset whose first use lands in a *later* commit, the horizontal `add the class` / `wire it up` split — is itself a trigger, and no proposed commit may be dead (the callee travels with its call site and its coverage; only a declared inert prerequisite — config key, off-by-default flag, additive expand migration — may precede its consumer, and code nothing in the range ever reaches belongs to the architecture / conformance walks, not here). Severity is fixed at Critical: never downgraded, never demoted into a refactoring section, never dropped by the Critical Findings Verification pass, never filtered on a late-iteration run. When the history is already atomic or the change is genuinely unsplittable, omit the section — never a "history is clean" confirmation. Canonical triggers, non-findings, gating, report shape, and the literal Suggested-Fix template: `@rules/code-review/general.mdc` *Commit Split & Atomic Deployability Proposal — canonical walk-through*.
...
- This skill is read-only and does not publish anywhere itself. The wrapper skills that consume its output (`@skills/code-review-github/SKILL.md`, `@skills/code-review-jira/SKILL.md`) **must** delegate the **single consolidated comment on every linked issue** in the originating tracker (GitHub issue, JIRA ticket, or both) to `@skills/pr-summary/SKILL.md` — the CR wrappers never author their own non-technical template. `pr-summary` produces a uniform *"TL;DR + Summary of changes + How to test"* comment understandable by non-technical project managers, rendered as GitHub Markdown for GitHub issues and JIRA Wiki Markup for JIRA tickets per `@rules/jira/general.mdc`. `@skills/assignment-compliance-check/SKILL.md` returns the **Functional review** markdown block on every run that has a linked tracker — including the affirmative `Goal met: Yes` checklist on a clean run — and the CR wrapper passes it as an embedded block to `pr-summary`, which appends it after `How to test` so the linked-tracker audience reads exactly **one comment per CR run** (per issue #498). Only when the compliance check returns the `no linked issue — assignment compliance skipped` status does the wrapper embed nothing. Technical findings still go directly on the PR comment.
- **`## Commit Split Proposal` section (issue #763).** Render it immediately after `## Findings` whenever the **Commit Split & Atomic Deployability Gate** fires: verdict, one row per proposed commit (subject, contents, source hunks, assignment item, cherry-pick verdict, reversibility), rollback notes, the reconciliation proving the union equals the current diff, and the blockers. The finding is **Critical**, listed in the `## Findings` Critical bucket by title with this section as its body, and exempt from the Faulty Example / Expected Behavior / Test Hint fields. Never dropped by the late-iteration filter; omitted entirely — section and summary slot — when the history is already atomic.
...
- **Late-iteration report scope (CR iteration > 2).** When the caller passes `iteration = <n>` (the Review loop iteration from `@skills/process-code-review/SKILL.md`) and `n > 2`, the report carries **Critical and Moderate findings only** — every Minor finding, `Refactoring (DRY / tech debt)`, and `Refactoring proposals` are dropped at Output assembly, `Counts` and `Summary` keep their real unchanged numbers, and the header gains the `Report scope:` line. No walk is skipped and the convergence gate is unchanged. Canonical: `@rules/code-review/general.mdc` *Late-Iteration Report Scope — Critical & Moderate Only (CR iteration > 2)*.
