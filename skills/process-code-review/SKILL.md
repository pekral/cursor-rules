...
- Apply the requested changes and keep scope limited to review feedback. All new or modified production code must follow @.claude/skills/class-refactoring/SKILL.md.
- Re-check current changes with @.claude/skills/code-review/SKILL.md and @.claude/skills/security-review/SKILL.md.  
- If review feedback requires additional tests, use @.claude/skills/create-missing-tests-in-pr/SKILL.md and ensure current changes are fully covered.
...
- Run the issue-tracker-specific code review skill before PR creation:
  - GitHub issue flow: run @.claude/skills/code-review-github/SKILL.md
  - JIRA issue flow: run @.claude/skills/code-review-jira/SKILL.md
...
- After the CR loop is clean (no **Critical** or **Moderate** findings), run @.claude/skills/test-like-human/SKILL.md when the change can be tested.
- Commit all changes and push the branch. If no pull request exists for the current branch, create one according to @.claude/rules/git/pr.mdc rules — link it to the original issue and follow the PR description format (title in English, body in the language of the assignment). Do not create a new PR before the CR cycle is clean.
...
- GitHub: run @.claude/skills/code-review-github/SKILL.md
- JIRA: run @.claude/skills/code-review-jira/SKILL.md
