...
**"Awaiting external input" findings are exempt from the reproducer requirement.** A finding whose Suggested Fix is the literal request-for-link template from `@rules/code-review/general.mdc` *Third-Party API & Service Documentation Verification (issue #748)* step 3 has no Faulty Example / Expected Behavior / Test Hint by nature — there is no code bug to reproduce, only a missing external source. Do **not** request a CR rerun for it and do **not** treat the missing fields as a blocker. Post the literal request as a reply on the PR (or add it to the PR description) and record it in the `cr-status` comment under a `## Awaiting external input` heading with the recorded reason — the same deferred-with-recorded-reason routing as a non-trivial pre-existing issue (see **Pre-fix phase** rule 5) — it stays open across iterations until the author supplies the link, at which point a later run applies the rule's **Gate** step and re-verifies.
