...
- **The Action returns a typed DTO for any structured result, never a raw array.** When the extracted use case hands back more than one value, declare a `final readonly` DTO under `app/Dto/<Domain>` with the `Data` suffix and return that — an associative array, a multi-key payload, or an array shape with named keys is never an Action return type, and the framework / vendor-contract exemption does not apply to a project-owned `__invoke()`. Scalars, `void`, an Eloquent model, and homogeneous lists / Collections of one type need no wrapper. Where the entry point previously built an array inline, the shaping moves to a Data Builder or a DTO named constructor and the controller serializes the DTO at the response boundary — the array does not survive the refactor (see `@rules/laravel/architecture.mdc` *Action return contract: a DTO, never an array*).
...
- Do not place inline validation inside the Action. Extract every validation-style guard — `instanceof` / null / range / amount checks that reject or skip input via `throw`, `return`, or `continue`, plus `throw_if()` / `throw_unless()` calls — into a dedicated Data Validator (default location `app/DataValidators/<Domain>/`, but follow the project's existing convention; `Pekral\Arch\DataValidation\DataValidator` trait when the package is installed). **Every validation rule set the refactor touches lives as a reusable trait in `app/Concerns/`** — Data Validators and the entry point's FormRequest both compose their rules from those traits, and a rule array is never left inline in `rules()`, in a Livewire `validate()` call, or in the Data Validator itself. Extract the trait even when only one class consumes it today (see `@rules/laravel/architecture.mdc` *Validation Rules (Traits)*).
...
- **The extracted Action's parameters, return type, and properties must be concrete, never `mixed`.** Normalize the input in the entry point's FormRequest / Data Validator / Data Builder instead of typing the Action's signature `mixed` (see `@rules/php/core-standards.mdc` PHP Practices and `@rules/laravel/architecture.mdc` *Action / Model Service / Repository / ModelManager receive normalized, typed data*).
...
10. Discover available fixers and checkers (prefer Phing targets from `build.xml`/`phing.xml`; fall back to Composer scripts in `composer.json`). Run fixers first, then checkers/analyzers on all changed files. Resolve all reported issues **by rewriting the flagged code, never by adding a suppression annotation** (`@rules/php/core-standards.mdc` PHP Practices).
